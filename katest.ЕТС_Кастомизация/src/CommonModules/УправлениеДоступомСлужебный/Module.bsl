...
Процедура ЕТС_ОбновитьРолиПользователей(Знач ОписаниеПользователей, Знач ПарольПользователяСервиса, ЕстьИзменения)
...
#Вставка
			...
			Для Каждого Стр Из ТекущиеНовыеРоли Цикл
				Если СтрНайти(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Стр.РольССылка,"Наименование"),"ЕТС администратор (Роль)") <> 0 Или
							СтрНайти(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Стр.РольССылка,"Наименование"),"ЕТС особый режим проведения (Роль)") <> 0 Или
							СтрНайти(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Стр.РольССылка,"Наименование"),"ЕТС все регионы (Роль)") <> 0 Тогда
					//Закомментаренный механизм, если роли будем искать по префиксу		
					//ВыбранныеДополнительныеРоли.Добавить(Стр.РольСсылка);
					ЗаполнитьЗначенияСвойств(НовыеРоли.Добавить(),Стр);
				КонецЕсли;
			КонецЦикла;
			//Для Каждого Стр Из ВыбранныеДополнительныеРоли Цикл
			//	НовыеРоли.Добавить().РольСсылка = Стр;
			//КонецЦикла;
#КонецВставки
		КонецЕсли;
